...
- Tasks 4.3 and 4.5 integration tests (which call `initDb`) fail at runtime due to a native module ABI mismatch: `better-sqlite3-multiple-ciphers` was compiled for NODE_MODULE_VERSION 137 (Node 20), but the environment runs Node 26.5.0 (NODE_MODULE_VERSION 147). This is a system-wide blocker affecting all 60+ tests in `db.test.ts` — not an imessage-sync code defect. Resolution: run `npm rebuild better-sqlite3-multiple-ciphers` with build tools available, or switch to Node 20/22.
